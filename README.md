...
The bounded live path also requires `PROOFSPEND_ADAPTER_MODE=arc-testnet`, the documented Circle developer-controlled wallet settings, two pre-created Arc Testnet wallets, and a writable persistent `PROOFSPEND_AUTH_STORE_PATH`. The server persists the exact approval before Circle submission and atomically consumes it during submission. Credentials are never written to that store. The file-backed store is intentionally limited to the single-instance judge demo; a multi-instance production deployment requires a transactional shared database.

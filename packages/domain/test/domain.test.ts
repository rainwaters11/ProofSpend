...
describe("separate state machines", () => {
  ...
  it("requires persisted approval and exact canonical intent evidence to submit", async () => {
    ...
    await expect(transitionApplicationSubmission({ ...submitted, submissionTransaction: { ...transaction, arcTransaction: { ...transaction.arcTransaction!, network: "OTHER_NETWORK" } } as typeof transaction })).rejects.toThrow(InvalidTransitionError);
  ...
  it("requires the exact authorized evaluator and confirmed evaluation evidence for terminal job decisions", () => {
    ...
    expect(() => transitionAgenticJob("SUBMITTED", "REJECTED", { ...context, aggregateId: rejected.jobId, actor, authorizedEvaluatorId: actor.actorId, jobEvidence: { ...rejected, transaction: { ...evaluationTransaction, network: "OTHER_NETWORK" } } as typeof rejected, jobApprovalDecision: approval("REJECTED"), jobEvaluationEvidence: evaluationEvidence("REJECTED") })).toThrow(InvalidTransitionError);

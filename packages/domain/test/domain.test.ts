...
describe("separate state machines", () => {
  ...
  it("enforces status-specific ERC-8183 evidence at the schema boundary", () => {
    ...
    for (const job of [open, funded, submitted, completed, rejected, mockJob("EXPIRED"), AgenticJobRefSchema.parse({ ...funded, status: "EXPIRED" }), AgenticJobRefSchema.parse({ ...submitted, status: "EXPIRED" })]) expect(AgenticJobRefSchema.parse(job)).toEqual(job);
  ...
  it("requires persisted approval and exact canonical intent evidence to submit", async () => {
    ...
    const submitForOperation = async (operationType: SubmissionOperationType, actorType: "FOUNDER" | "PROVIDER" | "EVALUATOR") => {
      ...
      const protocolTarget = operationType === "JOB_FUND" || operationType === "JOB_SUBMIT" || operationType === "JOB_EVALUATE" ? erc8183Target(operationType) : { kind: "DESTINATION" as const, destination: transaction.destinationReference, network: "ARC_TESTNET" as const, chainId: "synthetic:chain", isMock: true };
      ...
      return transitionApplicationSubmission({ ...submitted, submissionTransaction: candidateTransaction, executionBinding: candidateBinding, approvalDecision: candidateApproval, expectedExactIntentHash: candidateHash });
    };
  ...
  it("evidence-gates funding, submission, and expiry", async () => {
    ...
    expect((await transitionAgenticJob("OPEN", "EXPIRED", { ...context, occurredAt: open.expiresAt, aggregateId: open.jobId, actor: system, authorizedSystemId: system.actorId, currentJobEvidence: open })).status).toBe("EXPIRED");
    expect((await transitionAgenticJob("OPEN", "EXPIRED", { ...context, occurredAt: open.expiresAt, aggregateId: open.jobId, actor: system, authorizedSystemId: system.actorId, currentJobEvidence: open, jobEvidence: mockJob("EXPIRED") })).status).toBe("EXPIRED");
    ...
    for (const [terminal, currentJobEvidence] of [
      ...
      ["REJECTED", mockJob("REJECTED", mockTransaction("CONFIRMED", "JOB_EVALUATE"))],
      ["EXPIRED", mockJob("EXPIRED")],
...
describe("protocol-safe mocks and privacy", () => {
  ...
  it("transitions only mock jobs without contract behavior", async () => {
    ...
    const expired = AgenticJobRefSchema.parse({ ...job, status: "EXPIRED" });
    for (const staleTarget of [job, { ...expired, jobId: "mock:other" }, { ...expired, providerAddress: "mock:other-provider" }, { ...expired, deliverableReference: "mock:forged-deliverable" }, { ...expired, reasonReference: "mock:forged-reason" }, { ...expired, transaction: mockTransaction("CONFIRMED", "SETTLEMENT") }]) await expect(adapter.transition(job, "EXPIRED", { ...systemAuthority, jobEvidence: staleTarget })).rejects.toThrow(InvalidTransitionError);

...
describe("separate state machines", () => {
  ...
  it("requires the exact authorized evaluator and confirmed evaluation evidence for terminal job decisions", async () => {
    ...
    expect((await transitionAgenticJob("SUBMITTED", "COMPLETED", { ...context, aggregateId: completed.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: completed, jobApprovalDecision: approval("APPROVED"), jobEvaluationEvidence: evaluationEvidence("APPROVED") })).status).toBe("COMPLETED");
    await expect(transitionAgenticJob("SUBMITTED", "COMPLETED", { ...context, aggregateId: completed.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: unconfirmedSubmittedCurrent, jobEvidence: completed, jobApprovalDecision: approval("APPROVED"), jobEvaluationEvidence: evaluationEvidence("APPROVED") })).rejects.toThrow(InvalidTransitionError);
    await expect(transitionAgenticJob("SUBMITTED", "COMPLETED", { ...context, aggregateId: completed.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: { ...mockJob("SUBMITTED", mockTransaction("CONFIRMED", "JOB_SUBMIT")), transaction: { ...mockTransaction("CONFIRMED", "JOB_SUBMIT"), blockHash: null } } as unknown as ReturnType<typeof mockJob>, jobEvidence: completed, jobApprovalDecision: approval("APPROVED"), jobEvaluationEvidence: evaluationEvidence("APPROVED") })).rejects.toThrow(InvalidTransitionError);
    const rejected = mockJob("REJECTED", evaluationTransaction);
    expect((await transitionAgenticJob("SUBMITTED", "REJECTED", { ...context, aggregateId: rejected.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: rejected, jobApprovalDecision: approval("REJECTED"), jobEvaluationEvidence: evaluationEvidence("REJECTED") })).status).toBe("REJECTED");
    for (const transaction of [null, { ...evaluationTransaction, operationType: "JOB_SUBMIT" as const }, { ...evaluationTransaction, status: "SUBMITTED" as const, blockNumber: null, blockHash: null }]) await expect(transitionAgenticJob("SUBMITTED", "REJECTED", { ...context, aggregateId: rejected.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: { ...rejected, transaction }, jobApprovalDecision: approval("REJECTED"), jobEvaluationEvidence: evaluationEvidence("REJECTED") })).rejects.toThrow(InvalidTransitionError);
    await expect(transitionAgenticJob("SUBMITTED", "REJECTED", { ...context, aggregateId: rejected.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: { ...rejected, transaction: { ...evaluationTransaction, transactionHash: "mock:other" } }, jobApprovalDecision: approval("REJECTED"), jobEvaluationEvidence: evaluationEvidence("REJECTED") })).rejects.toThrow(InvalidTransitionError);
    await expect(transitionAgenticJob("SUBMITTED", "REJECTED", { ...context, aggregateId: rejected.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: { ...rejected, transaction: { ...evaluationTransaction, network: "OTHER_NETWORK" } } as unknown as typeof rejected, jobApprovalDecision: approval("REJECTED"), jobEvaluationEvidence: evaluationEvidence("REJECTED") })).rejects.toThrow(InvalidTransitionError);
    await expect(transitionAgenticJob("SUBMITTED", "REJECTED", { ...context, aggregateId: rejected.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: rejected, jobApprovalDecision: approval("REJECTED"), jobEvaluationEvidence: evaluationEvidence("REJECTED", { transactionChainId: "mock:chain" }) })).rejects.toThrow(InvalidTransitionError);
    await expect(transitionAgenticJob("SUBMITTED", "REJECTED", { ...context, aggregateId: rejected.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: rejected, jobApprovalDecision: approval("REJECTED", "mock:other"), jobEvaluationEvidence: evaluationEvidence("REJECTED") })).rejects.toThrow(InvalidTransitionError);
    await expect(transitionAgenticJob("SUBMITTED", "REJECTED", { ...context, aggregateId: rejected.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: rejected, jobApprovalDecision: approval("REJECTED", "mock:job", "mock:other"), jobEvaluationEvidence: evaluationEvidence("REJECTED") })).rejects.toThrow(InvalidTransitionError);
    await expect(transitionAgenticJob("SUBMITTED", "REJECTED", { ...context, aggregateId: rejected.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: rejected, jobApprovalDecision: approval("REJECTED", "mock:job", "mock:intent", `sha256:${"b".repeat(64)}`), jobEvaluationEvidence: evaluationEvidence("REJECTED") })).rejects.toThrow(InvalidTransitionError);
    await expect(transitionAgenticJob("SUBMITTED", "REJECTED", { ...context, aggregateId: rejected.jobId, actor: { actorId: "evaluator:other", actorType: "EVALUATOR" as const }, authorizedEvaluatorId: "evaluator:other", currentJobEvidence: submittedCurrent, jobEvidence: rejected, jobApprovalDecision: approval("REJECTED"), jobEvaluationEvidence: evaluationEvidence("REJECTED") })).rejects.toThrow(InvalidTransitionError);
    await expect(transitionAgenticJob("SUBMITTED", "COMPLETED", { ...context, aggregateId: completed.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: { ...completed, deliverableReference: "mock:replacement" }, jobApprovalDecision: approval("APPROVED"), jobEvaluationEvidence: evaluationEvidence("APPROVED") })).rejects.toThrow(InvalidTransitionError);
    await expect(transitionAgenticJob("SUBMITTED", "REJECTED", { ...context, aggregateId: rejected.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: { ...rejected, deliverableReference: "mock:replacement" }, jobApprovalDecision: approval("REJECTED"), jobEvaluationEvidence: evaluationEvidence("REJECTED") })).rejects.toThrow(InvalidTransitionError);
    for (const [targetState, decision, job] of [["COMPLETED", "APPROVED", completed], ["REJECTED", "REJECTED", rejected]] as const) {
      ...
      expect((await transitionAgenticJob("SUBMITTED", targetState, { ...context, aggregateId: job.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: job, jobApprovalDecision: approval(decision, "mock:job", "mock:intent", `sha256:${"a".repeat(64)}`, actor.actorId, "2025-12-31T23:59:59.000Z"), jobEvaluationEvidence: evidence })).status).toBe(targetState);
      expect((await transitionAgenticJob("SUBMITTED", targetState, { ...context, aggregateId: job.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: job, jobApprovalDecision: approval(decision), jobEvaluationEvidence: evidence })).status).toBe(targetState);
      await expect(transitionAgenticJob("SUBMITTED", targetState, { ...context, aggregateId: job.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: job, jobApprovalDecision: approval(decision, "mock:job", "mock:intent", `sha256:${"a".repeat(64)}`, actor.actorId, "2026-01-01T00:00:01.000Z"), jobEvaluationEvidence: evidence })).rejects.toThrow(InvalidTransitionError);
      await expect(transitionAgenticJob("SUBMITTED", targetState, { ...context, aggregateId: job.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: job, jobApprovalDecision: approval(decision, "mock:job", "mock:intent", `sha256:${"a".repeat(64)}`, actor.actorId, context.occurredAt, context.occurredAt), jobEvaluationEvidence: evidence })).rejects.toThrow(InvalidTransitionError);
      await expect(transitionAgenticJob("SUBMITTED", targetState, { ...context, aggregateId: job.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: job, jobApprovalDecision: approval(decision, "mock:job", "mock:intent", `sha256:${"a".repeat(64)}`, actor.actorId, "2025-12-31T23:59:59.000Z", context.occurredAt), jobEvaluationEvidence: evidence })).rejects.toThrow(InvalidTransitionError);
      await expect(transitionAgenticJob("SUBMITTED", targetState, { ...context, occurredAt: "invalid", aggregateId: job.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: job, jobApprovalDecision: approval(decision), jobEvaluationEvidence: evidence })).rejects.toThrow(InvalidTransitionError);
      await expect(transitionAgenticJob("SUBMITTED", targetState, { ...context, aggregateId: job.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: job, jobApprovalDecision: { ...approval(decision), decidedAt: "invalid" } as unknown as ReturnType<typeof approval>, jobEvaluationEvidence: evidence })).rejects.toThrow(InvalidTransitionError);

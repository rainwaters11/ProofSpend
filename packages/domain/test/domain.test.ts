...
describe("separate state machines", () => {
  ...
  it("requires the exact authorized evaluator and confirmed evaluation evidence for terminal job decisions", () => {
    ...
    expect(() => transitionAgenticJob("SUBMITTED", "REJECTED", { ...context, aggregateId: rejected.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: { ...rejected, transaction: { ...evaluationTransaction, transactionHash: "mock:other" } }, jobApprovalDecision: approval("REJECTED"), jobEvaluationEvidence: evaluationEvidence("REJECTED") })).toThrow(InvalidTransitionError);
    expect(() => transitionAgenticJob("SUBMITTED", "REJECTED", { ...context, aggregateId: rejected.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: { ...rejected, transaction: { ...evaluationTransaction, network: "OTHER_NETWORK" } } as unknown as typeof rejected, jobApprovalDecision: approval("REJECTED"), jobEvaluationEvidence: evaluationEvidence("REJECTED") })).toThrow(InvalidTransitionError);
    expect(() => transitionAgenticJob("SUBMITTED", "REJECTED", { ...context, aggregateId: rejected.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: rejected, jobApprovalDecision: approval("REJECTED"), jobEvaluationEvidence: evaluationEvidence("REJECTED", { transactionChainId: "mock:chain" }) })).toThrow(InvalidTransitionError);
    expect(() => transitionAgenticJob("SUBMITTED", "REJECTED", { ...context, aggregateId: rejected.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: rejected, jobApprovalDecision: approval("REJECTED", "mock:other"), jobEvaluationEvidence: evaluationEvidence("REJECTED") })).toThrow(InvalidTransitionError);
    expect(() => transitionAgenticJob("SUBMITTED", "REJECTED", { ...context, aggregateId: rejected.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: rejected, jobApprovalDecision: approval("REJECTED", "mock:job", "mock:other"), jobEvaluationEvidence: evaluationEvidence("REJECTED") })).toThrow(InvalidTransitionError);
    expect(() => transitionAgenticJob("SUBMITTED", "REJECTED", { ...context, aggregateId: rejected.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: rejected, jobApprovalDecision: approval("REJECTED", "mock:job", "mock:intent", `sha256:${"b".repeat(64)}`), jobEvaluationEvidence: evaluationEvidence("REJECTED") })).toThrow(InvalidTransitionError);
    expect(() => transitionAgenticJob("SUBMITTED", "REJECTED", { ...context, aggregateId: rejected.jobId, actor: { actorId: "evaluator:other", actorType: "EVALUATOR" as const }, authorizedEvaluatorId: "evaluator:other", currentJobEvidence: submittedCurrent, jobEvidence: rejected, jobApprovalDecision: approval("REJECTED"), jobEvaluationEvidence: evaluationEvidence("REJECTED") })).toThrow(InvalidTransitionError);
    expect(() => transitionAgenticJob("SUBMITTED", "COMPLETED", { ...context, aggregateId: completed.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: { ...completed, deliverableReference: "mock:replacement" }, jobApprovalDecision: approval("APPROVED"), jobEvaluationEvidence: evaluationEvidence("APPROVED") })).toThrow(InvalidTransitionError);
    expect(() => transitionAgenticJob("SUBMITTED", "REJECTED", { ...context, aggregateId: rejected.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: { ...rejected, deliverableReference: "mock:replacement" }, jobApprovalDecision: approval("REJECTED"), jobEvaluationEvidence: evaluationEvidence("REJECTED") })).toThrow(InvalidTransitionError);
    for (const [targetState, decision, job] of [["COMPLETED", "APPROVED", completed], ["REJECTED", "REJECTED", rejected]] as const) {
      ...
      expect(transitionAgenticJob("SUBMITTED", targetState, { ...context, aggregateId: job.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: job, jobApprovalDecision: approval(decision, "mock:job", "mock:intent", `sha256:${"a".repeat(64)}`, actor.actorId, "2025-12-31T23:59:59.000Z"), jobEvaluationEvidence: evidence }).status).toBe(targetState);
      expect(transitionAgenticJob("SUBMITTED", targetState, { ...context, aggregateId: job.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: job, jobApprovalDecision: approval(decision), jobEvaluationEvidence: evidence }).status).toBe(targetState);
      ...
      expect(() => transitionAgenticJob("SUBMITTED", targetState, { ...context, occurredAt: "invalid", aggregateId: job.jobId, actor, authorizedEvaluatorId: actor.actorId, currentJobEvidence: submittedCurrent, jobEvidence: job, jobApprovalDecision: approval(decision), jobEvaluationEvidence: evidence })).toThrow(InvalidTransitionError);
  ...
  it("evidence-gates funding, submission, and expiry", () => {
    ...
    expect(transitionAgenticJob("OPEN", "FUNDED", fundingContext).status).toBe("FUNDED");
    expect(() => transitionAgenticJob("OPEN", "FUNDED", { ...fundingContext, currentJobEvidence: undefined })).toThrow(InvalidTransitionError);
    expect(() => transitionAgenticJob("OPEN", "FUNDED", { ...fundingContext, currentJobEvidence: funded })).toThrow(InvalidTransitionError);
    ...
    ]) expect(() => transitionAgenticJob("OPEN", "FUNDED", { ...fundingContext, jobEvidence: { ...funded, ...changed } as unknown as typeof funded })).toThrow(InvalidTransitionError);
    expect(() => transitionAgenticJob("OPEN", "FUNDED", { ...fundingContext, jobEvidence: { ...funded, deliverableReference: "mock:deliverable" } })).toThrow(InvalidTransitionError);
    expect(() => transitionAgenticJob("OPEN", "FUNDED", { ...fundingContext, jobEvidence: { ...funded, reasonReference: "mock:reason" } })).toThrow(InvalidTransitionError);

    const submitted = mockJob("SUBMITTED", mockTransaction("SUBMITTED", "JOB_SUBMIT"));
    const submissionContext = { ...context, aggregateId: submitted.jobId, actor: adapter, authorizedAdapterId: adapter.actorId, currentJobEvidence: funded, jobEvidence: submitted };
    expect(transitionAgenticJob("FUNDED", "SUBMITTED", submissionContext).status).toBe("SUBMITTED");
    expect(() => transitionAgenticJob("FUNDED", "SUBMITTED", { ...submissionContext, jobEvidence: { ...submitted, deliverableReference: null } })).toThrow(InvalidTransitionError);
    expect(() => transitionAgenticJob("FUNDED", "SUBMITTED", { ...submissionContext, jobEvidence: { ...submitted, reasonReference: "mock:reason" } })).toThrow(InvalidTransitionError);
    ...
    for (const staleTarget of [mockJob("OPEN"), { ...mockJob("EXPIRED"), jobId: "mock:other" }, { ...mockJob("EXPIRED"), clientAddress: "mock:other-client" }]) expect(() => transitionAgenticJob("OPEN", "EXPIRED", { ...context, aggregateId: open.jobId, actor: system, authorizedSystemId: system.actorId, currentJobEvidence: open, jobEvidence: staleTarget })).toThrow(InvalidTransitionError);
...
describe("protocol-safe mocks and privacy", () => {
  ...
  it("transitions only mock jobs without contract behavior", () => {
    ...
    expect(adapter.transition(job, "FUNDED", { ...authority, currentJobEvidence: { ...job, budget: usdc("1") }, jobEvidence: funded }).job.status).toBe("FUNDED"); expect(job.status).toBe("OPEN");
    ...
    for (const staleTarget of [job, AgenticJobRefSchema.parse({ ...job, status: "EXPIRED", jobId: "mock:other" }), AgenticJobRefSchema.parse({ ...job, status: "EXPIRED", providerAddress: "mock:other-provider" })]) expect(() => adapter.transition(job, "EXPIRED", { ...systemAuthority, jobEvidence: staleTarget })).toThrow(InvalidTransitionError);

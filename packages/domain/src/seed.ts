import type { AllocationRule, DisclosurePreferences, LaunchVault, Milestone, MilestoneRequirement, Project, Reserve } from "./models";

const NOW = "2026-01-15T12:00:00.000Z";
const usd = (atomicUnits: string) => ({ asset: "USDC", atomicUnits });
...
export function createPawPovAiSeed(): PawPovAiSeed {
  ...
  const reserves = allocations.map(([id, name, amount]) => ({ id: `reserve:${id}`, vaultId: "vault:pawpovai", name, allocated: usd(amount), status: "PROPOSED" as const }));
  ...
  requirements.push({ id: "requirement:spend", milestoneId: "milestone:launch-ready", kind: "SPEND_LIMIT", description: "Eligible spend no greater than 150 test USDC", spendLimit: usd("150000000") });
  ...
  const seed: PawPovAiSeed = {
    ...
    vault: { id: "vault:pawpovai", projectId: "project:pawpovai", asset: "USDC", totalCapital: usd("1000000000"), mode: "MOCK", createdAt: NOW }, reserves,
    ...
    milestone: { id: "milestone:launch-ready", projectId: "project:pawpovai", title: "Launch identity and outreach ready", proposedAmount: usd("250000000"), status: "INCOMPLETE", requirementIds: requirements.map((r) => r.id), dueAt: null }, requirements,

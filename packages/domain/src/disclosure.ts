import { ProofOfProgressSchema, SettlementRecordSchema, type DisclosurePreferences, type EvidenceItem, type ProofOfProgress, type Project, type SettlementRecord } from "./models";
...
export function filterBackerDisclosure(input: {
  ...
  return {
    ...
    proofs: input.preferences.discloseProofRecords
      ? validatedProofs.filter((proof) => proof.projectId === input.preferences.projectId && input.preferences.approvedProofIds.includes(proof.id) && (proof.visibility === "BACKER_SHARED" || proof.visibility === "ONCHAIN_PUBLIC")).map(({ id, projectId, milestoneId, version, recordHash, createdAt }) => ({ id, projectId, milestoneId, version, recordHash, createdAt }))
      : [],

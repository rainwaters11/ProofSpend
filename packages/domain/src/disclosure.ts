import type { DisclosurePreferences, EvidenceItem, ProofOfProgress, Project, SettlementRecord } from "./models";
...
export function filterBackerDisclosure(input: {
  ...
  return {
    ...
    settlements: input.preferences.discloseSettlementState
      ? input.settlements.map(({ id, releaseRequestId, amount, state, updatedAt }) => ({ id, releaseRequestId, amount: structuredClone(amount), state, updatedAt }))
      : [],
  };
}

...
type SubmissionApprovalPolicy =
  | { actionKind: "RELEASE_APPROVAL"; actorType: "FOUNDER"; identifier: "authorizedApproverId"; targetKind: "DESTINATION" }
  | { actionKind: "JOB_EVALUATION" | "JOB_REJECTION"; actorType: "EVALUATOR"; identifier: "authorizedEvaluatorId"; targetKind: "ERC8183" };
function requiredSubmissionApprovalPolicy(operationType: NonNullable<TransactionRecord["arcTransaction"]>["operationType"]): SubmissionApprovalPolicy | null {
  if (operationType === "SETTLEMENT" || operationType === "REFUND") return { actionKind: "RELEASE_APPROVAL", actorType: "FOUNDER", identifier: "authorizedApproverId", targetKind: "DESTINATION" };
  if (operationType === "JOB_EVALUATE") return { actionKind: "JOB_EVALUATION", actorType: "EVALUATOR", identifier: "authorizedEvaluatorId", targetKind: "ERC8183" };
  if (operationType === "JOB_REJECT") return { actionKind: "JOB_REJECTION", actorType: "EVALUATOR", identifier: "authorizedEvaluatorId", targetKind: "ERC8183" };
  return null;
}
...
export async function transitionApplicationSubmission(context: TransitionContext) {
  ...
  const policy = arcTransaction === null ? null : requiredSubmissionApprovalPolicy(arcTransaction.operationType);
...
export async function transitionAgenticJob(from: AgenticJobStatus, to: AgenticJobStatus, context: TransitionContext) {
  ...
  const openClientRejection = from === "OPEN" && to === "REJECTED";
  if (!Number.isFinite(occurredAt) || !Number.isFinite(expiresAt) || (!openClientRejection && occurredAt >= expiresAt)) throw new InvalidTransitionError("agentic job expiry gate", from, to);
  if (to === "FUNDED") {

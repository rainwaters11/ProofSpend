...
export function transitionApplication(from: ProofSpendApplicationState, to: ProofSpendApplicationState, context: TransitionContext) {
  ...
  if (from === "SUBMITTED" && to === "CONFIRMED") {
    ...
    if (transaction === null || transaction.operationState !== "CONFIRMED" || transaction.arcTransaction?.status !== "CONFIRMED" || context.expectedTransactionId === undefined || context.expectedProjectId === undefined || context.expectedReleaseRequestId === undefined || context.aggregateId !== transaction.releaseRequestId || transaction.id !== context.expectedTransactionId || transaction.projectId !== context.expectedProjectId || transaction.releaseRequestId !== context.expectedReleaseRequestId || transaction.arcTransaction.operationType !== "SETTLEMENT") throw new InvalidTransitionError("ProofSpend application confirmation evidence", from, to);
  }
...
function sameImmutableJobTerms(current: AgenticJobRef, target: AgenticJobRef): boolean {
  return current.standard === target.standard && current.network === target.network && current.chainId === target.chainId && current.contractAddress === target.contractAddress && current.jobId === target.jobId && current.clientAddress === target.clientAddress && current.providerAddress === target.providerAddress && current.evaluatorAddress === target.evaluatorAddress && current.budget.asset === target.budget.asset && current.budget.atomicUnits === target.budget.atomicUnits && current.expiresAt === target.expiresAt && current.descriptionReference === target.descriptionReference && current.isMock === target.isMock;
}
function hasProviderSubmissionEvidence(job: AgenticJobRef): boolean {
  return job.deliverableReference !== null && job.transaction?.isMock === true && ["SUBMITTED", "CONFIRMED"].includes(job.transaction.status) && job.transaction.operationType === "JOB_SUBMIT" && job.transaction.transactionHash !== null;
}
function hasRefundEvidence(job: AgenticJobRef): boolean {
  return job.transaction?.isMock === true && job.transaction.status === "CONFIRMED" && job.transaction.operationType === "REFUND" && job.transaction.transactionHash !== null && job.transaction.blockNumber !== null && job.transaction.blockHash !== null;
}
export function transitionAgenticJob(from: AgenticJobStatus, to: AgenticJobStatus, context: TransitionContext) {
  ...
  if (to === "EXPIRED") {
    ...
    if (from !== "OPEN") {
      ...
      if (!target.success || !target.data.isMock || target.data.jobId !== context.aggregateId || target.data.status !== "EXPIRED" || !sameImmutableJobTerms(current.data, target.data) || target.data.deliverableReference !== current.data.deliverableReference || !hasRefundEvidence(target.data)) throw new InvalidTransitionError("agentic job expiry evidence", from, to);
    }
    return { status: to, auditEvent: event(context, from, to) } as const;
  }
  const current = AgenticJobRefSchema.safeParse(context.currentJobEvidence);
  const target = AgenticJobRefSchema.safeParse(context.jobEvidence);
  if (!current.success || !target.success || !current.data.isMock || !target.data.isMock || current.data.jobId !== context.aggregateId || current.data.status !== from || target.data.jobId !== context.aggregateId || target.data.status !== to || !sameImmutableJobTerms(current.data, target.data)) throw new InvalidTransitionError("agentic job lifecycle evidence", from, to);
  ...
  if (to === "COMPLETED" || to === "REJECTED") {
    ...
    if (!approval.success || !evaluationEvidence.success || approval.data.actionKind !== "JOB_EVALUATION" || approval.data.aggregateId !== target.data.jobId || approval.data.aggregateId !== context.aggregateId || approval.data.decision !== decision || approval.data.approver?.actorType !== "EVALUATOR" || approval.data.approver.actorId !== context.actor.actorId || approval.data.authorizedActorId !== context.authorizedEvaluatorId || decidedAt === null || occurredAt === null || expiresAt === null || decidedAt > occurredAt || occurredAt >= expiresAt || !hasProviderSubmissionEvidence(current.data) || target.data.deliverableReference !== current.data.deliverableReference || transaction === null || transaction.status !== "CONFIRMED" || transaction.operationType !== "JOB_EVALUATE" || transaction.transactionHash === null || evaluationEvidence.data.jobId !== target.data.jobId || evaluationEvidence.data.approvalId !== approval.data.id || evaluationEvidence.data.intentId !== approval.data.intentId || evaluationEvidence.data.exactIntentHash !== approval.data.exactIntentHash || evaluationEvidence.data.decision !== decision || evaluationEvidence.data.transactionHash !== transaction.transactionHash || evaluationEvidence.data.transactionNetwork !== transaction.network || evaluationEvidence.data.transactionChainId !== transaction.chainId || (to === "COMPLETED" && target.data.reasonReference !== null) || (to === "REJECTED" && target.data.reasonReference === null)) throw new InvalidTransitionError("agentic job evaluation evidence", from, to);
  }
  return { status: to, auditEvent: event(context, from, to) } as const;
}

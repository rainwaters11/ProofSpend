...
Issue #7's ADR-001 (`docs/architecture/adr-001-circle-execution-architecture.md`) selects **Circle Developer-Controlled Wallets** as the Arc Testnet custody and contract-execution path; the #2/#3/#4 architecture dependency gate is cleared and ADR-001 is accepted as the decision. Issue #7 acceptance still requires upgrading the repository runtime and CI to Node.js 22.6+ (the selected SDK's requirement; the repo currently documents and runs Node 20.18.2) and completing adapter implementation. Circle patterns may be researched earlier. Consider only `packages/circle-tools` and `kits/openai-agents` selectively; exclude `packages/agent-cli`, terminal UI, unrelated framework kits, and autonomous payments.
